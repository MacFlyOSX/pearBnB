...
const UpdateReview = () => {
  ...
  useEffect(() => {
    ...
    setAccRate(review.acc);
    setCheckRate(review.check);
    setLocRate(review.loc);
  ...
  const handleSubmit = async (e) => {
      ...
      const newReview = await dispatch(updateReview(reviewId, payload));
...
return (
  <div className='create-review-container'>
      ...
      <h1 className='create-review-header-title'>Rate & review: {review?.listing?.name}</h1>
      <div className='create-review-breakdown-section'>
          ...
          <div className='create-review-submit-button-section'>
              ...
              <button id='create-review-submit' onClick={handleSubmit}>Update Review</button>
          </div>
      </div>
  </div>
)
